...
def test_menu_principal_nueva_partida_flujo_completo():
    # 1) Nueva partida → 1) El Corazón de Ceniza → 1) dificultad Paseo
    salida: list[str] = []
    eleccion = menu_principal(
        entrada=EntradaTipeada(["1", "1", "1"]), salida=salida.append
    )
...
def test_menu_principal_con_presets_solo_pregunta_lo_que_falta():
    # con aventura y dificultad prefijadas, "1" llega directo a jugar
    eleccion = menu_principal(
        entrada=EntradaTipeada(["1"]),
...
def test_menu_principal_preset_personaje_invalido_avisa_y_pregunta():
    eleccion = menu_principal(
        entrada=EntradaTipeada(["1", "1"]),  # nueva → (personaje inválido → no hay lista; cae al héroe por defecto)
        salida=lambda _t: None,
        aventura="corazon_ceniza",
        dificultad="camino",
